...
exports.fenlei=function(data,pid=0,level=0){	
	var fenl=[];
	var _this=this;
	var fbdata=data;

	function doFenlei(fbdata,pid=0,level,name){
		for(var i=0;i<fbdata.length;i++){
			if(fbdata[i].pid==pid){
				fbdata[i]['parentName']=name;
				fbdata[i]['__v']=level;
				fenl.push(fbdata[i])
			
				doFenlei(fbdata,fbdata[i]._id,level+1,fbdata[i]['name']);
			}
		}
	}
	doFenlei(fbdata,pid,level+1,'<strong>顶级菜单<strong>');
	return fenl;
}
